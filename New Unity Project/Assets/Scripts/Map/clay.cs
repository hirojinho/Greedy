...
public class clay : MonoBehaviour
{
    ...
    void Update()
    {
        if(enemy.GetComponent<enemyPlaying>().playing == false)
        {
            ...
            if(l == 100 && exploring == false)
            {
                ...
                exploring = true;
                StartCoroutine(exploreClay(sphere[1]));
            }
        }
    }
    ...
    void OnTriggerStay(Collider other)
    {
        ...
        {
            isplayer = true;
            StartCoroutine(exploreClay(sphere[0]));
            exploring = true;
        }
    }
    
    IEnumerator exploreClay(GameObject sphere)
    {
        ...
        if(isplayer == true)
        {
            for(int i = 0; i < 10; i++)
            {
                player.GetComponent<resources>().clay += 5.0f;
