...
public class iron : MonoBehaviour
{
    ...
    void Update()
    {
        if(enemy.GetComponent<enemyPlaying>().playing == false)
        {
            ...
            if(l == 100 && exploring == false)
            {
                ...
                exploring = true;
                StartCoroutine(exploreIron(sphere[1]));
            }
        }
    }
    ...
    void OnTriggerStay(Collider other)
    {
        ...
        {
            isplayer = true;
            StartCoroutine(exploreIron(sphere[0]));
            exploring = true;
        }
    }
    
    IEnumerator exploreIron(GameObject sphere)
    {
        ...
        if(isplayer == true)
        {
            for(int i = 0; i < 10; i++)
            {
                player.GetComponent<resources>().iron += 5.0f;
