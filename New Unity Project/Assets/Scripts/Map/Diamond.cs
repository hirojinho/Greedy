...
public class Diamond : MonoBehaviour
{
    ...
    void Update()
    {
        if(enemy.GetComponent<enemyPlaying>().playing == false)
        {
            ...
            if(l == 100 && exploring == false)
            {
                ...
                exploring = true;
                StartCoroutine(exploreDiamond(sphere[1]));
            }
        }
    }
    ...
    void OnTriggerStay(Collider other)
    {
        ...
        {
            isplayer = true;
            StartCoroutine(exploreDiamond(sphere[0]));
            exploring = true;
        }
    }
    
    IEnumerator exploreDiamond(GameObject sphere)
    {
        ...
        if(isplayer == true)
        {
            for(int i = 0; i < 10; i++)
            {
                player.GetComponent<resources>().diamond += 5.0f;
