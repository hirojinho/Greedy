...
public class stone : MonoBehaviour
{
    ...
    void Update()
    {
        if(enemy.GetComponent<enemyPlaying>().playing == false)
        {
            ...
            if(l == 100 && exploring == false)
            {
                ...
                exploring = true;
                StartCoroutine(exploreStone(sphere[1]));
            }
        }
    }
    ...
    void OnTriggerStay(Collider other)
    {
        ...
        {
            isplayer = true;
            StartCoroutine(exploreStone(sphere[0]));
            exploring = true;
        }
    }
    
    IEnumerator exploreStone(GameObject sphere)
    {
        ...
        if(isplayer == true)
        {
            for(int i = 0; i < 10; i++)
            {
                player.GetComponent<resources>().stone += 5.0f;
