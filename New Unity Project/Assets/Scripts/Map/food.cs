...
public class food : MonoBehaviour
{
    ...
    void Update()
    {
        if(enemy.GetComponent<enemyPlaying>().playing == false)
        {
            ...
            if(l == 100 && exploring == false)
            {
                ...
                exploring = true;
                StartCoroutine(exploreFish(sphere[1]));
            }
        }
    }
    ...
    void OnTriggerStay(Collider other)
    {
        ...
        {
            isplayer = true;
            StartCoroutine(exploreFish(sphere[0]));
            exploring = true;
        }
    }
    
    IEnumerator exploreFish(GameObject sphere)
    {
        ...
        if(isplayer == true)
        {
            for(int i = 0; i < 10; i++)
            {
                player.GetComponent<resources>().fish += 5.0f;
